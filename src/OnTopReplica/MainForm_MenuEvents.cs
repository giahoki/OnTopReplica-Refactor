    ...
    partial class MainForm {
        ...
        private void Menu_Opacity_opening(object sender, CancelEventArgs e) {
            ...
            foreach (ToolStripMenuItem i in items) {
                // Skip items without a Tag (custom entry)
                if (i.Tag == null) {
                    i.Checked = false;
                    continue;
                }
        ...
        private void Menu_Opacity_custom(object sender, EventArgs e) {
            // Show a small dialog with a numeric up‑down for opacity (0‑100%)
            using (var dlg = new OpacityDialog(this.Opacity)) {
                if (dlg.ShowDialog(this) == DialogResult.OK) {
                    this.Opacity = dlg.OpacityPercent;
                    Program.Platform.OnFormStateChange(this);
                }
            }
        }
        ...
        private void Menu_AutoHideBorder_click(object sender, EventArgs e) {
            IsAutoHideBorderEnabled = autoHideBorderToolStripMenuItem.Checked;
        }
