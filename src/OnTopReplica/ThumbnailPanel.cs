...
namespace OnTopReplica {
    ...
    class ThumbnailPanel : Panel {
        ...
        protected byte ThumbnailOpacity {
            get {
                // Slightly increase brightness in crop mode for better visibility
                return _drawMouseRegions ? (byte)245 : (byte)255;
            }
        }
        ...
        private void UpdateThumbnail() {
            if (_thumbnail != null && !_thumbnail.IsInvalid){
                try {
                    ...
                    _padWidth = (int)Math.Round((Size.Width - _thumbnailSize.Width) / 2.0);
                    _padHeight = (int)Math.Round((Size.Height - _thumbnailSize.Height) / 2.0);
